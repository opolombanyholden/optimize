...
@section('title', 'Nouvelle qualification')
...
<div class="page-header d-flex justify-content-between align-items-center">
    <div>
        ...
        <p class="text-muted mb-0">Enregistrer un diplome ou une certification</p>
    </div>
...
<form action="{{ route('rh.qualifications.store') }}" method="POST">
    @csrf
            ...
            <div class="row g-3">
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select name="niveau" id="niveau" class="form-select tom-select-rh @error('niveau') is-invalid @enderror">
                        ...
                        @foreach(\App\Models\Referentiel\NiveauQualification::actif()->orderBy('ordre')->orderBy('libelle')->get() as $n)
                            <option value="{{ $n->libelle }}" {{ old('niveau') === $n->libelle ? 'selected' : '' }}>{{ $n->libelle }}</option>
                        @endforeach
                    </select>
