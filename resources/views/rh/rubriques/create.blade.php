...
<div class="page-header d-flex justify-content-between align-items-center">
    <div>
        <h1>Nouvelle rubrique de paie</h1>
        <p class="text-muted mb-0">Definir une nouvelle ligne de paie (gain, retenue ou cotisation)</p>
    </div>
...
<form action="{{ route('rh.rubriques.store') }}" method="POST">
    @csrf
        ...
        <div class="card-body">
            <div class="row g-3">
                <div class="col-12 col-md-3">
                    ...
                    <input type="text" name="code" id="code" class="form-control @error('code') is-invalid @enderror" value="{{ old('code') }}" maxlength="20" required>
                    @error('code')
                ...
                <div class="col-12 col-md-3">
                    ...
                    <input type="number" min="0" name="ordre_affichage" id="ordre_affichage" class="form-control @error('ordre_affichage') is-invalid @enderror" value="{{ old('ordre_affichage', 0) }}">
                ...
                <div class="col-12 col-md-4">
                    ...
                    <select name="type" id="type" class="form-select @error('type') is-invalid @enderror" required>
                        ...
                        <option value="gain" {{ old('type') == 'gain' ? 'selected' : '' }}>Gain</option>
                        <option value="retenue" {{ old('type') == 'retenue' ? 'selected' : '' }}>Retenue</option>
                        <option value="cotisation" {{ old('type') == 'cotisation' ? 'selected' : '' }}>Cotisation</option>
                    </select>
                    @error('type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-12 col-md-4">
                    ...
                    <select name="base_calcul" id="base_calcul" class="form-select @error('base_calcul') is-invalid @enderror" required>
                        <option value="">-- Selectionner --</option>
                        <option value="fixe" {{ old('base_calcul') == 'fixe' ? 'selected' : '' }}>Montant fixe</option>
                        <option value="pourcentage" {{ old('base_calcul') == 'pourcentage' ? 'selected' : '' }}>Pourcentage</option>
                        <option value="formule" {{ old('base_calcul') == 'formule' ? 'selected' : '' }}>Formule</option>
                    </select>
                    @error('base_calcul')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                {{-- Champs conditionnels selon base_calcul --}}
                <div class="col-12 col-md-6 calcul-pourcentage" style="display:none;">
                    <label for="taux" class="form-label">Taux (%)</label>
                    <input type="number" step="0.01" min="0" name="taux" id="taux" class="form-control @error('taux') is-invalid @enderror" value="{{ old('taux') }}">
                    @error('taux')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-12 col-md-6 calcul-fixe" style="display:none;">
                    ...
                    <input type="number" step="0.01" min="0" name="montant_fixe" id="montant_fixe" class="form-control @error('montant_fixe') is-invalid @enderror" value="{{ old('montant_fixe') }}">
                    @error('montant_fixe')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-12 calcul-formule" style="display:none;">
                    ...
                    <textarea name="formule" id="formule" class="form-control @error('formule') is-invalid @enderror" rows="3" placeholder="Ex: salaire_base * 0.05">{{ old('formule') }}</textarea>
                ...
                <div class="col-12 col-md-6">
                    <div class="form-check mt-2">
                        ...
                        <input type="checkbox" name="imposable" id="imposable" value="1" class="form-check-input" {{ old('imposable') ? 'checked' : '' }}>
                ...
                <div class="col-12 col-md-6">
                    <div class="form-check mt-2">
                        ...
                        <input type="checkbox" name="cotisable" id="cotisable" value="1" class="form-check-input" {{ old('cotisable') ? 'checked' : '' }}>
