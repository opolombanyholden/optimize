...
@section('title', 'Modifier evenement de carriere')
...
<div class="page-header d-flex justify-content-between align-items-center">
    <div>
        ...
        <p class="text-muted mb-0">{{ $evenement->libelle }} &mdash; {{ $evenement->employee?->noms }} {{ $evenement->employee?->prenoms }}</p>
    </div>
...
<form action="{{ route('rh.evenements-carriere.update', $evenement) }}" method="POST">
    @csrf
    @method('PUT')
        ...
        <div class="card-body">
            <div class="row g-3">
                <div class="col-12 col-md-6">
                    ...
                    <input type="hidden" name="employee_id" value="{{ $evenement->employee_id }}">

                    <select id="employee_id" class="form-select" disabled>
                        @foreach($employees as $employee)
                            <option value="{{ $employee->id }}" {{ $evenement->employee_id == $employee->id ? 'selected' : '' }}>{{ $employee->noms }} {{ $employee->prenoms }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select name="typesevenementscarriere_id" id="typesevenementscarriere_id" class="form-select tom-select-rh @error('typesevenementscarriere_id') is-invalid @enderror" required>
                        ...
                        @foreach(\App\Models\TypeEvenementCarriere::actif()->orderBy('ordre')->orderBy('libelle')->get() as $type)
                            <option value="{{ $type->id }}" {{ old('typesevenementscarriere_id', $evenement->typesevenementscarriere_id) == $type->id ? 'selected' : '' }}>{{ $type->libelle }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-12 col-md-8">
                    ...
                    <input type="text" name="libelle" id="libelle" class="form-control @error('libelle') is-invalid @enderror" value="{{ old('libelle', $evenement->libelle) }}" required>
                    @error('libelle')
                ...
                <div class="col-12 col-md-4">
                    ...
                    <input type="date" name="date_effet" id="date_effet" class="form-control @error('date_effet') is-invalid @enderror" value="{{ old('date_effet', $evenement->date_effet?->format('Y-m-d')) }}" required>
                    @error('date_effet')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select name="ancien_poste" id="ancien_poste" class="form-select tom-select-rh @error('ancien_poste') is-invalid @enderror">
                        <option value="">-- Selectionner --</option>
                        @php $curAnc = old('ancien_poste', $evenement->ancien_poste); @endphp
                        @foreach($postesAct as $p)
                            <option value="{{ $p->libelle }}" {{ $curAnc === $p->libelle ? 'selected' : '' }}>{{ $p->libelle }}</option>
                        @endforeach
                        @if($curAnc && !$postesAct->pluck('libelle')->contains($curAnc))
                            <option value="{{ $curAnc }}" selected>{{ $curAnc }} (legacy)</option>
                        @endif
                    </select>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select name="nouveau_poste" id="nouveau_poste" class="form-select tom-select-rh @error('nouveau_poste') is-invalid @enderror">
                        <option value="">-- Selectionner --</option>
                        @php $curNew = old('nouveau_poste', $evenement->nouveau_poste); @endphp
                        @foreach($postesAct as $p)
                            <option value="{{ $p->libelle }}" {{ $curNew === $p->libelle ? 'selected' : '' }}>{{ $p->libelle }}</option>
                        @endforeach
                        @if($curNew && !$postesAct->pluck('libelle')->contains($curNew))
                            <option value="{{ $curNew }}" selected>{{ $curNew }} (legacy)</option>
                        @endif
                    </select>
                    <small class="form-text text-muted">Géré dans <a href="{{ route('admin.referentiel-rh.index', 'postes') }}" target="_blank">Admin › Référentiels</a></small>
                ...
                <div class="col-12">
                    ...
                    <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" rows="4">{{ old('description', $evenement->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
